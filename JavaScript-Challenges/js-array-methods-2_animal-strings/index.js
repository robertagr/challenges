...
const firstAnimalStartingWithLetterP = animalStrings.find((animals) =>
  animals.startsWith("p")
);
console.log("firstAnimalStartingWithLetterP:", firstAnimalStartingWithLetterP);

const indexOfGiraffe = animalStrings.findIndex((animals) =>
  animals.startsWith("giraffe")
);
...
const anyAnimalEndsWithLetterZ = animalStrings.find((animals) =>
  animals.endsWith("z")
);
...
const lengthOfAllAnimalCharacters = animalStrings.map(
  (animals) => animals.length
);

const sumOfAllAnimalCharacters = lengthOfAllAnimalCharacters.reduce(
  (a, b) => a + b
);
